...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		if (word1.length() == 0 || word2.length() == 0 || word1.equals("") || word2.equals("") || word1.equals(word2)) {
			return path;
		}
		...
		while (q.size() > 0) {
			...
			List<String> oneAway = nw.distanceOne(curr.getWord(), true);
			for (String w : oneAway) {
				if (!visited.contains(w)) {
					WPTreeNode wtn = curr.addChild(w);
					q.addLast(wtn);
					visited.add(w);
					if (w.equals(word2)) {
						return wtn.buildPathToRoot();
					}
				}
			}
			System.out.println(printQueue(curr.getChildren()));
		}
		return null;
	}
