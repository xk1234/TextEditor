...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
			for (int i = 0; i < s.length(); i++) {
				String newStr = s.substring(0, i) + (char)charCode + s.substring(i, s.length());
				if (!currentList.contains(newStr) && 
						(!wordsOnly||dict.isWord(newStr))) {
					currentList.add(newStr);
				}
			}
			String last = s + (char) charCode;
			if (!currentList.contains(last) && 
					(!wordsOnly||dict.isWord(last))) {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (queue.size() > 0 && retList.size() < numSuggestions) {
			...
			List<String> nei = distanceOne(curr, true);
			for (String w: nei) {
				if (!visited.contains(w)) {
					visited.add(w);
					((LinkedList<String>) queue).addLast(w);
					if (dict.isWord(w)) {
						retList.add(w);
					}
				}
